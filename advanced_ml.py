...
def train_and_persist():
    ...
    # Optimized CatBoost Parameters (from best Optuna trials)
    ...
    artifacts = {
        ...
        'metrics_snapshot': {
            ...
            'f1': f1_score(y_test, (full_pipeline.predict_proba(X_test)[:, 1] >= best_threshold).astype(int)),
            'recall': recall_score(y_test, (full_pipeline.predict_proba(X_test)[:, 1] >= best_threshold).astype(int))
        }
    }
...
if __name__ == "__main__":
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Pipeline Training Complete.")
    print(f"Artifacts saved to 'models/churn_model_v3.joblib'")
    print(f"Test Recall: {results['metrics_snapshot']['recall']:.4f} | F1: {results['metrics_snapshot']['f1']:.4f}")
